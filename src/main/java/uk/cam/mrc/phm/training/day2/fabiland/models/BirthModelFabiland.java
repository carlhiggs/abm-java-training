...
public class BirthModelFabiland extends AbstractModel implements BirthModel {
    ...
    public BirthModelFabiland(DataContainer dataContainer, PersonFactory factory,
    ...
    void giveBirth(Person person) {
        ...
        ((PersonHealthFabiland)child).updateWeeklyMarginalMetHours(Mode.walk, 0.f);
        ((PersonHealthFabiland)child).updateWeeklyMarginalMetHours(Mode.bicycle, 0.f);
        ((PersonHealthFabiland)child).setWeeklyMarginalMetHoursSport(0.f);

        Map<String, Float> exposureMap = new HashMap<>();
        exposureMap.put("pm2.5", 0.f);
        exposureMap.put("no2", 0.f);

        ((PersonHealthFabiland)child).updateWeeklyPollutionExposures(exposureMap);
        ...
        for (HealthExposures exposures : HealthExposures.values()){
            ...
            ((PersonHealthFabiland)child).getRelativeRisksByDisease().put(exposures, rr);
        }

        ((PersonHealthFabiland) child).getHealthDiseaseTracker().put(currentYear, Arrays.asList("healthy"));
